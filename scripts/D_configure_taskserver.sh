...
configure_taskserver_ciphers() {
	output=$(taskd config --force ciphers NORMAL)
	echo $output
}
...
modify_taskd_service_file(){
	...
	swapline_containing_string "Group=" "Group=$LINUX_USERNAME" "src/taskd.service"
...
configure_taskserver_config_ciphers() {
	output=$(yes | task config taskd.ciphers -- NORMAL)
	echo $output
	# TODO: change the ~ to some variable of the user path
}
